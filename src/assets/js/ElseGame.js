...
export default class ElseGame {
  constructor() {
    ...
    if(sessionStorage.getItem('model') === null)
      sessionStorage.setItem('model','Policeman');
    if(sessionStorage.getItem('color') === null)
      sessionStorage.setItem('color','Black');

    this.g_room = sessionStorage.getItem('room');
    this.g_username = sessionStorage.getItem('username');
    this.g_model = sessionStorage.getItem('model');
    this.g_color = sessionStorage.getItem('color');
    ...
    options.assets.push(`${game.assetsPath}fbx/town_old.fbx`);
  ...
  // addGallary() {
  //   let cubeMaterial = new THREE.MeshPhongMaterial({ color: 0xF1F79E });
  //   let texture = new THREE.TextureLoader();
  //   let wallTexture = texture.load(`${this.assetsPath}images/plaster-diffuse.jpg`);
  //   const wallmaterial = new THREE.MeshBasicMaterial({
  //     map: wallTexture // 纹理贴图
  //   })
  //   for (var i = 0; i < galleryInfo.length; i++) {
  //     var res = galleryInfo[i];
  //     var wallGeometry = new THREE.BoxGeometry(res.wallX, res.wallY, res.wallZ);
  //     var wall = new THREE.Mesh(wallGeometry, wallmaterial);
  //     // position the wall
  //     wall.position.x = res.x;
  //     wall.position.y = res.wallY / 2.0;
  //     wall.position.z = res.z;
  //     this.scene.add(wall);
  //
  //     var paintGeometry = new THREE.BoxGeometry(res.paintCubeX, res.paintCubeY, res.paintCubeZ);
  //     let paintTexture = texture.load(`${this.assetsPath}images/${res.id}.jpg`);
  //     let paintMaterial = new THREE.MeshBasicMaterial({ map: paintTexture });
  //     let paintMaterials = [
  //       cubeMaterial,
  //       cubeMaterial,
  //       cubeMaterial,
  ...
  //       cubeMaterial
  //     ];
  //     paintMaterials[res.faceid] = paintMaterial;
  //     const paintMesh = new THREE.Mesh(paintGeometry, paintMaterials);
  //     paintMesh.position.x = res.x + res.offX;
  //     paintMesh.position.y = res.paintY;
  //     paintMesh.position.z = res.z + res.offZ;
  ...
  init() {
    ...
    this.player = new ElsePlayerLocal(this);
    //this.createNPC();
  ...
  // createNPC() {
  //   this.npc = new ElsePlayer(this, null, true);
  //
  // }
  ...
  loadEnvironment(loader) {
    const game = this;
    loader.load(`${this.assetsPath}fbx/town_old.fbx`, function (object) {
  ...
  //   const txt = document.getElementById('message_text');
  //   var mxDis2 = 1000.0 * 1000.0;
  //
  //   var findPaintID = -1;
  //   //遍历所有注册进入数组的对象
  //   for (var i = 0; i < galleryInfo.length; i++) {
  //     var paint = galleryInfo[i];
  //     var dis2 = (paint.x - x) * (paint.x - x) + (paint.z - z) * (paint.z - z)
  //     if (dis2 < mxDis2) { findPaintID = i; mxDis2 = dis2; }
  //   }
  //   //如果并不在任何名画附近则不用显示
  //   if (findPaintID == -1) {
  //     panel.style.display = 'none';
  ...
  //     //这一类对应的是刚刚进入名画附近
  //     title.innerHTML = galleryInfo[findPaintID].title;
  //     title.style.fontSize = '20px';
  //     txt.innerHTML = galleryInfo[findPaintID].description;
  ...
  // showNPCInfo(x, z) {
  //   const panel = document.getElementById('npc');
  //   const txt = document.getElementById('npc_message');
  //   var dis2 = (NPCInitLocation[0] - x) * (NPCInitLocation[0] - x) + (NPCInitLocation[2] - z) * (NPCInitLocation[2] - z);
  //   var mxDis2 = 750.0 * 750.0;
  //   if (dis2 > mxDis2) {
  //     this.NPCstate = 'far'; console.log('far');
  //   } else {
  //     if (this.preNPCstate === undefined || this.preNPCstate == 'far') {
  //       this.NPCstate = 'near';
  //       console.log('near');
  //     }
  //
  ...
  //
  //
  //   //如果状态机的状态没有发生变化就不进行处理
  //   if (this.NPCstate == this.preNPCstate) return;
  //   //console.log('are you ok??');
  //   if (this.NPCstate == 'near') {
  //     //console.log('into near');
  //     txt.innerHTML = chatMessage[0];
  //     txt.style.fontSize = '16px';
  //     panel.style.display = 'flex';
  //
  //   } else if (this.NPCstate == 'chat0') {
  //     txt.innerHTML = chatMessage[1];
  //
  //   } else if (this.NPCstate == 'chat1') {
  //     txt.innerHTML = chatMessage[2];
  //
  //   } else if (this.NPCstate == 'chat2') {
  //     txt.innerHTML = chatMessage[3];
  //
  //   } else {
  //     this.NPCstate = 'far';
  //     panel.style.display = 'none';
  //   }
  //
  //   this.preNPCstate = this.NPCstate;
  //
  // }
      ...
      if (game.player.id != data.id) {
        ...
        if (iplayer === undefined) {
          ...
          if (rplayer === undefined) {
            //Initialise player
            game.initialisingPlayers.push(new ElsePlayer(game, data));
          } else {
            //Player exists
  ...
  //   if (this.NPCstate == 'near') {
  //     if (event.key === "f") this.NPCstate = 'chat0';
  //
  //   } else if (this.NPCstate == 'chat0') {
  //     if (event.key === "1") this.NPCstate = 'chat1';
  //     else if (event.key === "2") this.NPCstate = 'chat2';
  //     else this.NPCstate = 'near';
  //   } else if (this.NPCstate == 'chat1') {
  //     this.NPCstate = 'chat0';
  //
  //   } else if (this.NPCstate == 'chat2') {
  ...
  animate() {
    ...
    this.animateCount++;
    if (this.player != undefined && this.player.object != undefined && this.animateCount % 100 == 0) {
      const pos = this.player.object.position.clone();
      //this.showPaintInfo(pos.x, pos.z);
      //this.showNPCInfo(pos.x, pos.z);
    }
    ...
    //this.npc.action = 'Running';
    // if (Math.floor(Math.random() * 100) == 0) {
    // 	this.npc.action = 'Idle';
    // }
...
class ElsePlayer {
  constructor(game, options, isNPC) {
    this.local = true;
    let model, colour;
    ...
    if (options === undefined || options == null) {
      ...
      if (isNPC == true) model = 'BusinessMan';
      else model = game.g_model;
    ...
    loader.load(`${game.assetsPath}fbx/people/${model}.fbx`, function (object) {
      ...
      if (isNPC == true) {
        ...
        player.object.position.set(NPCInitLocation[0], NPCInitLocation[1], NPCInitLocation[2]);
        player.object.rotation.set(0, 3.14, 0);
      } else {
        player.object.position.set(LocalPlayerInitLocation[0], LocalPlayerInitLocation[1], LocalPlayerInitLocation[2]);
        player.object.rotation.set(0, 1, 0);
      }
...
class ElsePlayerLocal extends ElsePlayer {
  constructor(game, model) {
    ...
    socket.emit('set username', game.g_username);
    socket.emit('join room', game.g_room);
